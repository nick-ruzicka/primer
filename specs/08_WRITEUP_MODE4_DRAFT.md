...
Pre-call briefings for enterprise AEs, built in 48 hours as a portfolio project.
...
The MCP protocol is the right abstraction here. Each source system gets a server that exposes read-only tools. If  swaps Catalyst for Gainsight tomorrow, we swap the Catalyst MCP server. If RevTech ships the unified layer in 18 months, we point the servers at the new endpoints. The rest of the system is invariant.
...
## Section 10 — The 90-day plan if I joined 

**The skill library isn't something you write in a month. It's something you mine.**

 has years of Gong call recordings. Inside those calls is the accumulated knowledge of what works in every sales and CSM motion — locked in individual recordings, not organizational memory.
...
This is the concrete answer to "what would you do in this role." Not a generic pitch — a three-phase plan with measurable outputs each phase, grounded in the specific asset  has (the Gong corpus) that most competitors don't.
...
**Primer is live at [primer.yourdomain.com]. Source at [github.com/nick-ruzicka/primer-].**
