...
| 2026-04-22 02:12 | 7 | .github/workflows/ci.yml | PASS | YAML parses (PyYAML). 4 jobs defined: python (ruff), frontend (tsc --noEmit + biome), shellcheck (all *.sh), nginx (nginx -t in Docker). `concurrency` group cancels stale runs; `timeout-minutes` caps every job. |
