...
SERVER_MODULE = "mcp_servers.exa_mcp.server"
...
async def main() -> int:
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print(f"[exa-mcp] tools: {tool_names}")
            ...
            if failures:
                print(f"\n[exa-mcp] {len(failures)} tool(s) failed")
                return 1
            print(f"\n[exa-mcp] all {len(tool_names)} tool(s) ok")
            return 0
