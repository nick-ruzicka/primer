"""Smoke test for salesforce-mcp.

Run from the repo root:

    uv run python -m mcp_servers.salesforce_mcp.test

Spins up the server over stdio, lists tools, then calls every tool against
`northstar_beauty` and prints the results. Exits non-zero if any tool errors.
"""
...
SERVER_MODULE = "mcp_servers.salesforce_mcp.server"
...
async def main() -> int:
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print(f"[salesforce-mcp] tools: {tool_names}")
            ...
            if failures:
                print(f"\n[salesforce-mcp] {len(failures)} tool(s) failed")
                return 1
            print(f"\n[salesforce-mcp] all {len(tool_names)} tool(s) ok")
            return 0
